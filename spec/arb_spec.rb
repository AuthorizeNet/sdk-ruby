...
describe AuthorizeNet::ARB::Transaction do
  ...
  before do
    ...
    @subscription = AuthorizeNet::ARB::Subscription.new(
      ...
      subscription_id: nil,
      customer: AuthorizeNet::Customer.new(email: rand(10_000_000).to_s + 'pb@mail.com', id: rand(10_000_000).to_s),
  ...
  it "should be able to retrieve list of subscriptions" do
    ...
    if response.subscription_details.nil?
  ...
  it "should return successful response for valid subscription list search types and sortBy fields" do
    ...
    %i[cardExpiringThisMonth subscriptionActive subscriptionExpiringThisMonth
       subscriptionInactive].each do |searchType|
      # iterate over valid sortBy fields
      %i[id name status createTimeStampUTC lastName firstName accountNumber amount
         pastOccurrences].each do |sortBy|
...
describe AuthorizeNet::ARB::Subscription do
  before do
    @attributes = %i[name length unit start_date total_occurrences trial_occurrences amount trial_amount invoice_number description subscription_id]
    @attribute_values = {
      name: "daily foo",
      length: 1,
